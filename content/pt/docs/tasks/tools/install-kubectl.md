...
reviewers:
- rpkatz 
title: Instalação e configuração do kubectl
content_type: task
weight: 10
card:
  name: tasks
  weight: 20
  title: Instalação do kubectl
---

<!-- overview -->
A ferramenta de linha de comando do Kubernetes, [kubectl](/docs/reference/kubectl/kubectl/),
permite a execução de comandos em clusters Kubernetes.
Você pode usar kubectl para implantar aplicações, inspecionar e gerenciar recursos do cluster,
e analisar logs. Para uma lista completa das operações do kubuectl, veja [Overview of kubectl](/docs/reference/kubectl/overview/).

## {{% heading "prerequisites" %}}

Você deve usar a versão kubectl que esteja uma versão secundária atrás de diferença da versão do teu cluster.
Por exemplo, um cliente usando v1.2 deve funcionar com v.1.1, v1.2, e v1.3 master.
Usar a última versão do kubectl ajuda a evitar problemas desconhecidos.

<!-- steps -->

## Instalação do kubectl no Linux

### Instalação do binário kubectl com curl no Linux

1. Faça download da última versão com o comando:
   ...
   {{< note >}}
Para fazer download de uma versão específica, substitua a parte `$(curl -L -s https://dl.k8s.io/release/stable.txt)` do comando com a versão específica.
...
1. Valide o binário (opcional)
   ...
   Faça download do arquivo checksum do kubectl:

   ```bash
   curl -LO "https://dl.k8s.io/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl.sha256"
   ```

   Valide o binário do kubectl com o arquivo checksum:

   ```bash
   echo "$(<kubectl.sha256) kubectl" | sha256sum --check
   ```

   Se válido, o resultado é:

   ```bash
   kubectl: OK
   ```

   Se a verificação falhar, `sha256`  existe com status nonzero e tem a saída similar a:
   ...
   {{< note >}}
   Faça download da mesma versão tanto para o binário quanto para o checksum.
   {{< /note >}}
...
1. Instale kubectl
   ...
   {{< note >}}
   Se você não tem acesso root no sistema, você pode instalar kubectl no diretório `~/.local/bin`:
...
1. Teste para assegurar que a versão instalada está atualizada:

   ```bash
   kubectl version --client
   ```

### Instalação usando pacote de gerenciamento nativo

{{< tabs name="kubectl_install" >}}
{{< tab name="Ubuntu, Debian ou HypriotOS" codelang="bash" >}}
...
{{< tab name="CentOS, RHEL ou Fedora" codelang="bash" >}}cat <<EOF > /etc/yum.repos.d/kubernetes.repo
[kubernetes]
name=Kubernetes
...
### Instalação usando outro pacote de gerenciamento

{{< tabs name="other_kubectl_install" >}}
{{% tab name="Snap" %}}
Se está usando Ubuntu ou outra distribuição Linux que suporta o gerenciador de pacotes [snap](https://snapcraft.io/docs/core/install), kubectl está disponível como aplicação em [snap](https://snapcraft.io/).
...
{{% tab name="Homebrew" %}}
Se você está usando Linux com [Homebrew](https://docs.brew.sh/Homebrew-on-Linux) como gerenciador de pacotes, kubectl está disponível para [instalação](https://docs.brew.sh/Homebrew-on-Linux#install).
...
### Instale o binário kubectl com curl no macOS

1. Faça downdolad da última versão:
   ...
   Por exemplo, para fazer download da versão {{< param "fullversion" >}} no macOS, digite:
